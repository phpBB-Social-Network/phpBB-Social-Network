	...
	$.fn.snEditable = function(options) {
		...
		this.map(function(idx) {
			...
			if ($.metadata) {
				...
				if (meta.input != undefined) obj.editType = meta.input;
				if (meta.name != undefined) obj.editName = meta.name;
				if (meta.value != undefined) obj.editValue = meta.value;
				if (meta.max != undefined) obj.editMax = meta.max;
				if (meta.data != undefined) {
					obj.editOptions = th.attr('class').replace(/^.*?data\s*?:\s*?([\[\{]{1}.*?[\}\]]{1})[^\}\]]*?.*?$/, '$1');
					eval('obj\.data = ' + obj.editOptions + ';');
				}
				if (meta.value != undefined) this.edit = meta.value;
			...
			if (obj.editType == undefined || obj.editName == undefined || (obj.editType == 'select' && obj.editOptions == undefined)) { return; }
