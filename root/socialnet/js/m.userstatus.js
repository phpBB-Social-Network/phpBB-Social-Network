	...
	$.sn.us = {
		...
		init : function(opts) {
					...
					$.ajax({
						...
						data : {
							...
							page : {
								...
								image : bImage ? '' : $('.sn-us-fetchImgs img:visible').attr('src'),
								imageH : bImage ? '' : $('.sn-us-fetchImgs img:visible').attr('imgH'),
								imageW : bImage ? '' : $('.sn-us-fetchImgs img:visible').attr('imgW'),
					...
					success : function(data) {
						...
						if (data.images.length == 0) {
							...
							$.each(data.images, function(i, image) {
								$('<img src="' + image.img + '" width="100" id="sn-us-fetchPreviewImg_' + i + '" imgH="' + image.height + '" imgW="' + image.width + '" ' + (i > 0 ? 'style="display:none;max-height:150px" ' : '') + '/>').appendTo('.sn-us-fetchImgs');
							});
							$('.sn-us-fetchImages').show();
							$('.sn-us-thumbs .sn-us-thumbsImg').show();
						}
						$('.sn-us-fetchImages .sn-us-fetchThumb .mPic').html(data.images.length);
			...
			$('#sn-us-noVideo').change(function() {
				...
				$('.sn-us-fetchVideoInfo').toggle();
				$('.sn-us-fetchVideoProvider').toggle();
			});
			...
			$('.sn-us-videoOverlay').live('click', function() {
				...
				/*
				.children('embed').removeAttr('style').attr({
					height : $(obj).attr('height'),
					width : $(obj).attr('width')
				});*/
